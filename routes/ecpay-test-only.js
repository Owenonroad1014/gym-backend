...
router.get('/', function (req, res) {
  ...
  const amount = Number(req.query.amount) || 0    //orderid要改這裡
  ...
  const ClientBackURL = 'http://localhost:3000/carts/complete' //前端成功頁面
...
router.post('/api', async (req, res) => {
  ...
  try {
    ...
    const updateValues = [
      PaymentDate,
      TradeNo,
      MerchantTradeNo.replace('od', '') // 將 MerchantTradeNo 轉換為 orderId
    ];
